...
def process_features(features):
    ...
    for feature in feature_lambda(
        features, feature_test_type, {"types": ALLOWED_FEATURES}, subfeatures=True
    ):
        cleaned_subfeatures = []
...
def gff_filter(gff3):
    for rec in gffParse(gff3):
        cleaned_features = sorted(
            list(process_features(rec.features)), key=lambda x: x.location.start
        )
        rec.features = sorted(
            list(add_exons(cleaned_features)), key=lambda x: x.location.start
        )
        rec.annotations = {}
