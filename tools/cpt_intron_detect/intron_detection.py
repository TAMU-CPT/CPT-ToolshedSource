...
class IntronFinder(object):
    ...
    def output_gff3(self, clusters):
        ...
        for cluster_idx, cluster_id in enumerate(clusters):
            ...
            mRNA.location = FeatureLocation(
                mRNA.location.start, mRNA.location.end, cds.location.strand
            )
            gene.sub_features = [mRNA]
            gene.location = FeatureLocation(
                gene.location.start, gene.location.end, cds.location.strand
            )
